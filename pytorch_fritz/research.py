...
class WeightNorm:
    def __init__(self, in_features, out_features, alpha=0.01, kekl=False):
    ...
    def forward(self, input, deterministic=False, tangent=False, gauss=False):
        W_norm = self.internal_weight / self.internal_weight.norm(dim=0)
        big_dot_products = []
        if self.kekl:
            variances = []
            for kek in np.logspace(-5, 5, 20):
                self.alpha=kek
                ...
                for i in range(250):
                    ...
                    # distances = ((W_noisy_normed - W_norm) ** 2).sum(dim=1)**0.5
                    # dot_products += distances.data.cpu().numpy().tolist()

                    dot_products_here = W_norm.transpose(1, 0).mm(W_noisy_normed).diag().data.cpu().numpy()
                    dot_products_here[dot_products_here > 1] = 1.0
                    dot_products += np.arccos(dot_products_here).tolist()
                big_dot_products.append(dot_products)
                ...
                # variances.append(np.std(normed_activations)**2)
            #np.save('variances', np.array(variances))
            np.save('dot_products', np.array(big_dot_products))
        else:
...
class Net(nn.Module):
    def __init__(self, input_size, num_classes, alpha=0.01):
        ...
        self.fc2 = WeightNorm(1024, 1024, alpha, kekl=True)
